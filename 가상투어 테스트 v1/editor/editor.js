...
const PinGate = (() => {
  ...
  function submit() {
    ...
    if (input.value === tourData.settings.security.editorPin) {
      ...
      setTimeout(() => {
        gate.remove();
        if (_onSuccess) _onSuccess(); // ✅ PIN 통과 후 에디터 초기화
      }, 300);
    } else {
...
const Editor = (() => {
  ...
  function renderCurrentScene() {
    ...
    // 에디터에서는 _previewUrl(objectURL) 우선, 없으면 panoSrc(파일 경로) 사용
    const src = scene._previewUrl || scene.panoSrc || '';

    // Marzipano 없거나 소스 없으면 그라디언트 폴백
  ...
  function exportJSON() {
    // _previewUrl(objectURL)은 에디터 전용 — JSON 내보낼 때 제거
...
const PhotoModal = (() => {
  ...
  /* ── 확정 ──
     파일은 실제로 assets/panos/, assets/thumbs/ 에 직접 배치해야 합니다.
     에디터는 경로만 JSON에 저장하고, 미리보기용 objectURL은 _previewUrl에 별도 유지합니다.
  ── */
  function confirm() {
    ...
    const ids = _pending.map((pf, i) => {
      ...
      grp.scenes.push({
        ...
        thumb:       thumbPath,
        panoSrc:     panoPath,
        _previewUrl: pf.objectUrl,  // 에디터 내 미리보기 전용 (JSON 내보내기 시 제외)
        markers:     []
      });
      return id;
    });
    ...
    _pending = [];  // objectUrl은 _previewUrl로 씬에 이관됨
    document.getElementById('modal-add-photo').style.display = 'none';
  }
